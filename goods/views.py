...
class SannerGoodsTagView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user:
            if bar_code is None:
                ...
                return ListModel.objects.filter(
                    openid=self.request.auth.openid, bar_code=bar_code, is_delete=False
                )
        else:
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        good_detail = AsnDetailModel.objects.filter(
            asn_code=data, goods_code=serializer.data["goods_code"]
        ).first()
...
class APIViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user:
            ...
            if search_word:
                if id is None:
                    data_list = ListModel.objects.filter(
                        openid=self.request.auth.openid, is_delete=False
                    )
                    search_list = data_list.filter(
                        Q(goods_shape=search_word) | Q(goods_specs=search_word)
                    )
                    return search_list
                else:
                    data_list = ListModel.objects.filter(
                        openid=self.request.auth.openid, id=id, is_delete=False
                    )
                    search_list = data_list.filter(
                        Q(goods_shape=search_word) | Q(goods_specs=search_word)
                    )
    ...
    def create(self, request, *args, **kwargs):
        ...
        data["unit_volume"] = round(
            (float(data["goods_w"]) * float(data["goods_d"]) * float(data["goods_h"]))
            / 1000000000,
            4,
        )
        if ListModel.objects.filter(
            openid=data["openid"], goods_code=data["goods_code"], is_delete=False
        ).exists():
                        ...
                        if goods_brand.objects.filter(
                            ...
                            if goods_color.objects.filter(
                                ...
                                if goods_shape.objects.filter(
                                    ...
                                    raise APIException(
                                        {
                                            "detail": "Goods Shape does not exists or it has been changed"
                                        }
                                    )
                            else:
                                raise APIException(
                                    {
                                        "detail": "Goods Color does not exists or it has been changed"
                                    }
                                )
                        else:
                            raise APIException(
                                {
                                    "detail": "Goods Brand does not exists or it has been changed"
                                }
                            )
                    else:
                        raise APIException(
                            {
                                "detail": "Goods Class does not exists or it has been changed"
                            }
                        )
                else:
                    raise APIException(
                        {"detail": "Goods Unit does not exists or it has been changed"}
                    )
            else:
                raise APIException(
                    {"detail": "Supplier does not exists or it has been changed"}
                )
                                    ...
                                    if goods_specs.objects.filter(
                                        ...
                                        if goods_origin.objects.filter(
                                            ...
                                            scanner.objects.filter(
                                                ...
                                                mode="GOODS",
                                                code=qs.goods_code,
                                            ).update(code=str(data["goods_code"]))
...
class FileDownloadView(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        response[
            "Content-Disposition"
        ] = "attachment; filename='goodslist_{}.csv'".format(
